...
namespace WebCalendar.Mappers
{
    public static class DomainToModel
    {
        ...
        {
            return new Event() { ID = ev.ID, Title = ev.Title, BeginTime = ev.BeginTime, EndTime = ev.EndTime, EventColor = ev.Color, Description = ev.Description, CalendarID = ev.CalendarID };
        }

        internal static EventViewModel Map(Event ev)
        {
            return new EventViewModel() { ID = ev.ID, Title = ev.Title, Description = ev.Description, BeginTime = ev.BeginTime, Color = ev.EventColor, EndTime = ev.EndTime, CalendarID = ev.CalendarID};
        }
